...
if action==REQUEST
	...
	p.addParameter('ReceiveBufferSize',8192,@isscalar);
    p.parse(varargin{:});
    host = p.Results.host;
    port = p.Results.port;
    serialize = p.Results.serialize;
	receiveBufferSize = p.Results.ReceiveBufferSize;
...
elseif action==ACCEPT
    ...
    p.addOptional('serialize',true, @(x) ismember(x,[0 1]) || islogical(x));
    p.addOptional('timeout',1000, @isnumeric);
	p.addOptional('ReceiveBufferSize',8192,@isscalar);
    p.parse(varargin{:});
    port = p.Results.port;
    serialize = p.Results.serialize;
	receiveBufferSize = p.Results.ReceiveBufferSize;
...
if action == REQUEST
    jTcpObj = jtcp_request_connection(host,port,timeout,serialize,receiveBufferSize);
elseif action == ACCEPT
    jTcpObj = jtcp_accept_connection(port,timeout,serialize,receiveBufferSize);
elseif action == WRITE
...
%-------------------------------------------------------------------------
function jTcpObj = jtcp_request_connection(host,port,timeout,serialize,receiveBufferSize)
%
...
%-------------------------------------------------------------------------
function jTcpObj = jtcp_accept_connection(port,timeout,serialize,receiveBufferSize)
%
...
try
    ...
    serverSocket.close;
    %rethrow(ME);
	error('jtcp:connectionAcceptFailed','%s.m--Failed to make TCP connection.\nJava error message follows:\n%s',mfilename,ME.message);
end % try
